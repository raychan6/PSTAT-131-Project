...
* The column names were then cleaned. A separate data set that preserves the `Differential` variable was created to aid with EDA. Then, it was dropped from the original data set.
...
We can see that Jokic and the Nuggets have won more games at home but have lost more games at away. This may be a sign of the existence of home court advantage.
...
This graphic shows that Jokic and the Nuggets win with the greatest point disparity against the New York Knicks. However, they lose with the greatest point disparity against the Golden State Warriors. 

## Correlation Between Stats
A correlation matrix was created for the numeric variables. A separate data set with no missing values was created for this section. 128 observations were omitted through this process, so this may have a slight negative impact on the accuracy of this analysis.
...
`year` is strongly positively correlated with `fg`, `fga`, `tfb` (total rebounds), `ast` (assists), `pts`, `gm_sc`, `ast_percent` (assist percentage), and `usg_percent` (usage percentage). This suggests that Jokic improves offensively as time goes on. He is also more involved in team plays and provides more assists as time goes on. 

`usg_percent` is also strongly positively correlated with `fg`, `fga`, and `pts`. As he becomes more involved in the team's plays, he attempts and makes more shots. However, `usg_percent` is also moderately correlated with `tov` (turnovers). 
...
Finally, `differential` is slightly positively correlated with `ast`, `ts_percent` (true shooting percentage), `efg_percent` (effective field goal percentage), and `trb_percent` (total rebound percentage). Therefore, it may be possible that when Jokic has more assists, has better overall shooting numbers, and grabs more rebounds that are available, it may lead to a win. What is interesting is that despite Jokic being praised as one of the passers in the NBA today (and most likely in NBA history by the time he retires), assists and assist percentage are not strongly positively correlated with differential. 
...
This graphic shows that Jokic's average scoring efficiency (considering 2-pointers, 3-pointers, and free throws) has generally been improving. Even when there was a decline from 2017 to 2019, his average `ts_percent` was still above 0.5875. This means that he was successful in over half of all of his scoring attempts! In particular, we can see that Jokic's average true shooting percentage was over a ridiculously elite percentage of 65% in 2021 and 2022, which is when he won his two MVP awards. Therefore, there has generally been an upward trend during his career so far regarding his performance. 
...
I created the recipe used to fit the models after identifying which predictors had missing values.
...
To compare model performances, I created a table that displays the accuracy rates for all four models using the training set and then using the testing set. 
...
models <- c("Logistic Regression", "SVM", "Random Forest", "Boosted Trees")
results <- tibble(testing_accuracy = accuracies, models = models)
results %>% 
  arrange(-accuracies)
```
